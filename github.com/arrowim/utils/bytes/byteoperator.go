...
//byte转int8
//**/
////func Bytes2Bool(data []byte, start int) (error, bool) {
...
func Bytes2HexString2(data []byte, mark string) string {
	if len(data) == 0 {
		return ""
	}
...
//func HexString2Bytes(data string) []byte {
//	s := []byte(data)
//	buf := bytes.NewBuffer([]byte{})
//	for i := 0; i <= len(s)-2; i += 2 {
//		d := string(s[i : i+2])
//		f, _ := strconv.ParseInt(string(d), 16, 16)
//		//		fmt.Println(d)
...
func checkDataLength(data []byte, start int, length int) error {
	if len(data)-start < length || start < 0 {
		return errors.New(fmt.Sprintf("数据长度不对,data len(%d),start(%d), need len(%d),data(%s)\n",
			len(data),
			start, length, Bytes2HexString(data)))
	}
	return nil
}
